...
extension ISO_32000.WinAnsiEncoding {
    /// Sanitize a Unicode string to ASCII
    ///
    /// Replaces fancy typography with ASCII equivalents for maximum compatibility:
    ...
    /// - Copyright → (C)
    /// - Registered → (R)
    /// - fi/fl ligatures → fi/fl
    /// - Other non-ASCII → ?
    ///
    /// ## Usage
    ///
    /// ```swift
    /// let input = "Hello "World" — Pro™"
    /// let output = ISO_32000.WinAnsiEncoding.sanitize(input)
    /// // "Hello \"World\" -- Pro(TM)"
    /// ```
    ///
    /// - Parameter string: The Unicode string to sanitize
    /// - Returns: An ASCII-safe string
    public static func sanitize(_ string: String) -> String {
        var result = ""
        result.reserveCapacity(string.count)

        for scalar in string.unicodeScalars {
            // First check if there's an ASCII replacement for typography
            if let replacement = asciiReplacement(for: scalar) {
                result += replacement
            } else if scalar.isASCII {
                // Keep ASCII characters as-is
                result.unicodeScalars.append(scalar)
            } else {
                // Non-ASCII without replacement becomes ?
                result += "?"
            }
        }
    ...
    /// Get ASCII replacement for a Unicode scalar
    private static func asciiReplacement(for scalar: Unicode.Scalar) -> String? {
        switch scalar.value {
        // Smart quotes → straight quotes
        case 0x201C, 0x201D:  // left/right double quotes
            return "\""
        case 0x2018, 0x2019:  // left/right single quotes
            return "'"
        ...
        case 0x2035:  // reversed prime
            return "`"
        case 0x00AB:  // guillemotleft
            return "<<"
        case 0x00BB:  // guillemotright
            return ">>"
    ...
    /// Check if a string can be encoded without loss
    ///
    /// - Parameter string: The string to check
    /// - Returns: `true` if all characters can be encoded
    public static func canEncode(_ string: String) -> Bool {
        for scalar in string.unicodeScalars {
            if !canEncode(scalar) {
                return false
            }
