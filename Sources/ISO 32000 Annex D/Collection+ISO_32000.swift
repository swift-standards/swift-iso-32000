...
// MARK: - String Initializers per Encoding

extension String {
    /// Initialize from WinAnsiEncoding bytes
    ///
    /// - Parameter bytes: The WinAnsi-encoded bytes
    /// - Returns: The decoded string, or `nil` if any byte is invalid
    @inlinable
    public init?<Bytes: Collection>(winAnsi bytes: Bytes) where Bytes.Element == UInt8 {
        self.init(ISO_32000.WinAnsiEncoding.self, bytes: bytes)
    }

    /// Initialize from PDFDocEncoding bytes
    ///
    /// - Parameter bytes: The PDFDoc-encoded bytes
    /// - Returns: The decoded string, or `nil` if any byte is invalid
    @inlinable
    public init?<Bytes: Collection>(pdfDoc bytes: Bytes) where Bytes.Element == UInt8 {
        self.init(ISO_32000.PDFDocEncoding.self, bytes: bytes)
    }

    /// Initialize from StandardEncoding bytes
    ///
    /// - Parameter bytes: The Standard-encoded bytes
    /// - Returns: The decoded string, or `nil` if any byte is invalid
    @inlinable
    public init?<Bytes: Collection>(standard bytes: Bytes) where Bytes.Element == UInt8 {
        self.init(ISO_32000.StandardEncoding.self, bytes: bytes)
    }

    /// Initialize from MacRomanEncoding bytes
    ///
    /// - Parameter bytes: The MacRoman-encoded bytes
    /// - Returns: The decoded string, or `nil` if any byte is invalid
    @inlinable
    public init?<Bytes: Collection>(macRoman bytes: Bytes) where Bytes.Element == UInt8 {
        self.init(ISO_32000.MacRomanEncoding.self, bytes: bytes)
    }

    /// Initialize from MacExpertEncoding bytes
    ///
    /// - Parameter bytes: The MacExpert-encoded bytes
    /// - Returns: The decoded string, or `nil` if any byte is invalid
    @inlinable
    public init?<Bytes: Collection>(macExpert bytes: Bytes) where Bytes.Element == UInt8 {
        self.init(ISO_32000.MacExpertEncoding.self, bytes: bytes)
    }

    /// Initialize from SymbolEncoding bytes
    ///
    /// - Parameter bytes: The Symbol-encoded bytes
    /// - Returns: The decoded string, or `nil` if any byte is invalid
    @inlinable
    public init?<Bytes: Collection>(symbol bytes: Bytes) where Bytes.Element == UInt8 {
        self.init(ISO_32000.SymbolEncoding.self, bytes: bytes)
    }

    /// Initialize from ZapfDingbatsEncoding bytes
    ///
    /// - Parameter bytes: The ZapfDingbats-encoded bytes
    /// - Returns: The decoded string, or `nil` if any byte is invalid
    @inlinable
    public init?<Bytes: Collection>(zapfDingbats bytes: Bytes) where Bytes.Element == UInt8 {
        self.init(ISO_32000.ZapfDingbatsEncoding.self, bytes: bytes)
    }
}
